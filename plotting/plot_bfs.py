#!/usr/bin/env python3
"""
BFS plotting script for wide CSV:

CSV format (wide):
- First column: System (or Test/Row/Name accepted)
- Remaining columns: graph names/paths (e.g., lj1_directed_hdr.txt, ...)

Cell values: runtime in ms (consistent definition across systems)

By default, plots *speedup over CPU* (CPU baseline = 1.0).
...
def prettify_col_label(col: str, style: str) -> str:
    """
    style:
      - 'as_is'   : keep column header
...
def main():
    ...
    ap.add_argument("--csv", default="bfs_results.csv", help="CSV file path or '-' for stdin.")
    ap.add_argument("--out", default="bfs_results.pdf")
    ap.add_argument("--figsize", default="10,4.2", help="W,H in inches")
    ...
    ap.add_argument("--title", default="BFS: speedup over CPU across graphs")
    ap.add_argument("--xlabel", default="Graph")
    ap.add_argument("--ylabel", default="Speedup over CPU")
    ap.add_argument("--ylim", default="1e-3,1e2", help="ymin,ymax (log scale). Tune to your data.")
    ...
    # x-label formatting
    ap.add_argument("--xlabels", choices=["as_is", "basename", "stem"], default="stem",
                    help="How to display graph names on x-axis (default: stem).")
    ...
    ap.add_argument("--bar-gap", type=float, default=0.22)
    ...
    # Graph columns are everything except system_col
    graph_cols = [c for c in df.columns if c != system_col]
    if not graph_cols:
        raise ValueError("CSV must have graph columns after the System column.")

    # Keep the original column order as x-axis order
    graph_cols_str = [str(c) for c in graph_cols]
    df = df[[system_col] + graph_cols_str]

    for c in graph_cols_str:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    # Build per-system runtime vectors
    data_rt: Dict[str, np.ndarray] = {}
    for _, row in df.iterrows():
        sysname = str(row[system_col])
        vals = row[graph_cols_str].to_numpy(dtype=float)
    ...
    if mode == "speedup":
        ...
        for s in order:
            if s == baseline:
                v = np.ones_like(base, dtype=float)
                v = np.where(np.isnan(base), np.nan, v)
                data_plot[s] = v
            else:
    ...
    # X positions (graphs)
    x = np.arange(len(graph_cols_str), dtype=float)

    # Bars per group (systems)
    K = len(order)
    ...
    # Log scale for both modes (keeps style consistent). If you want linear for runtime, tweak here.
    ax.set_yscale("log")
    ...
    # X tick labels (graphs)
    xlabels = [prettify_col_label(c, args.xlabels) for c in graph_cols_str]
    ax.set_xticks(x)
    ...
    if mode == "runtime":
        ax.set_ylabel("Runtime (ms)", fontsize=args.label_fontsize)
    else:
    ...
    # NA marker if all systems are NaN for a given graph
